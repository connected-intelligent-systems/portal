...
const TimeBasedPermissionShow = ({ permission }: { permission: any }) => {
  ...
  const formatOperator = (operator: string) => {
    switch (operator) {
      case 'odrl:gt': return translate('resources.policies.show.after');
      case 'odrl:lt': return translate('resources.policies.show.before');
      case 'odrl:eq': return translate('resources.policies.show.at');
      ...
      <CardContent sx={{ py: 1 }}>
        ...
        <Grid container spacing={1}>
          <Grid item xs={12} sm={4}>
            ...
            <Box sx={{ mt: 0.5 }}>
              <Chip label={permission.action || 'odrl:use'} color="success" variant="outlined" size="small" />
            </Box>
          </Grid>
...
const IdentityBasedPermissionShow = ({ permission }: { permission: any }) => {
  ...
  const formatOperator = (operator: string) => {
    switch (operator) {
      case 'odrl:eq': return translate('resources.policies.show.equals');
      case 'odrl:neq': return translate('resources.policies.show.notEquals');
      case 'odrl:isNoneOf': return translate('resources.policies.show.isNoneOf');
      case 'odrl:isPartOf': return translate('resources.policies.show.isPartOf');
      default: return operator;
    }
  };
        ...
        {permission.constraints && permission.constraints.length > 0 && (
          <Box sx={{ mt: 1 }}>
            ...
            {permission.constraints.map((constraint: any, index: number) => (
              <Box key={index} sx={{ p: 1, bgcolor: 'grey.50', borderRadius: 1, mb: 0.5, mt: 0.5 }}>
                <Grid container spacing={1}>
                  <Grid item xs={4}>
                    <Typography variant="caption" color="text.secondary" display="block">
                      {translate('resources.policies.show.leftOperand')}
                    </Typography>
                    <Typography variant="body2" sx={{ mt: 0.25 }}>{constraint.leftOperand}</Typography>
                  </Grid>
                  <Grid item xs={4}>
                    <Typography variant="caption" color="text.secondary" display="block">
                      {translate('resources.policies.show.operator')}
                    </Typography>
                    <Typography variant="body2" sx={{ mt: 0.25 }}>{constraint.operator}</Typography>
                  </Grid>
                  <Grid item xs={4}>
                    <Typography variant="caption" color="text.secondary" display="block">
...
const PermissionRenderer = ({ permission }: { permission: any }) => {
  if (!permission || !permission.constraints || permission.constraints.length === 0) {
          ...
          return (
            <Box>
              ...
              <Box sx={{ mb: 2 }}>
                ...
                <Box sx={{ mt: 0.5 }}>
                  {record?.policyType ? (
                    <Chip label={record.policyType} color="secondary" variant="outlined" size="small" />
                  ) : (
                    <Typography variant="body2" color="text.secondary">-</Typography>
                  )}
                </Box>
              ...
              <Box sx={{ mb: 2 }}>
                ...
                <Typography variant="h6" sx={{ mt: 0.5 }}>{record?.name || '-'}</Typography>
              </Box>
              ...
              <Box sx={{ mb: 3 }}>
                ...
                <Typography variant="body2" sx={{ mt: 0.5 }}>
                  {record?.description || translate('resources.policies.show.noDescription')}
                </Typography>
              </Box>

              {/* Tabs for Rules */}
              <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
                <Tabs value={activeTab} onChange={handleTabChange} aria-label="policy rules tabs">
                  <Tab 
                    label={translate("resources.policies.show.permissions")} 
                    icon={<SecurityIcon />}
                    disabled={permissions.length === 0}
                    sx={{ color: permissions.length === 0 ? 'text.disabled' : undefined }}
                  />
                  <Tab 
                    label={translate("resources.policies.show.obligations")} 
                    icon={<PolicyIcon />}
                    disabled={obligations.length === 0}
                    sx={{ color: obligations.length === 0 ? 'text.disabled' : undefined }}
                  />
                  <Tab 
                    label={translate("resources.policies.show.prohibitions")} 
                    icon={<BlockIcon />}
                    disabled={prohibitions.length === 0}
                    sx={{ color: prohibitions.length === 0 ? 'text.disabled' : undefined }}
                  />
                </Tabs>
              </Box>
