  ...
  return (
    <Box>
      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
        Versioning and metadata information based on Dublin Core Terms and OWL
        vocabularies. For more information, see{" "}
        <Link
      ...
      <Labeled fullWidth label="Creator">
        <FunctionField
          render={(record: any) =>
            record?.properties?.["http://purl.org/dc/terms/creator"]?.[
      ...
      <Labeled fullWidth label="Created">
        <FunctionField
          render={(record: any) => {
            const created =
              record?.properties?.["http://purl.org/dc/terms/created"];
      ...
      <Labeled fullWidth label="Modified">
        <FunctionField
          render={(record: any) => {
            const modified =
              record?.properties?.["http://purl.org/dc/terms/modified"];
      ...
      <Labeled fullWidth label="Previous Versions">
        <FunctionField
          render={(record: any) => {
            const versions =
              record?.properties?.["http://purl.org/dc/terms/hasVersion"];
            if (!versions) {
              ...
              <Box sx={{ pl: 2 }}>
                {versionsArray.map((version: any, index: number) => (
                  ...
                  >
                    ...
                    >
                      Issued:{" "}
                      {version["http://purl.org/dc/terms/issued"]
                        ? new Date(
                            version["http://purl.org/dc/terms/issued"]
                          ).toLocaleDateString()
                        : "-"}
